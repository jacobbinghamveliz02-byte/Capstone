import { SupervisionCCReport, } from "@zwave-js/cc";
import { ZWaveError, ZWaveErrorCodes, mergeSupervisionResults, } from "@zwave-js/core";
/** Creates a wrapper that looks like an instance of a specific CC API, but can handle multiple instances of that API */
export function createMultiCCAPIWrapper(apiInstances) {
    if (apiInstances.length === 0) {
        throw new ZWaveError("At least one CC API instance must be provided", ZWaveErrorCodes.Argument_Invalid);
    }
    else if (apiInstances.some((a) => a.ccId !== apiInstances[0].ccId)) {
        throw new ZWaveError("All CC API instances must be for the same CC", ZWaveErrorCodes.Argument_Invalid);
    }
