...
import { CommandClasses, EncapsulationFlags, MPANState, MessagePriority, NODE_ID_BROADCAST, NODE_ID_BROADCAST_LR, SPANState, SecurityClass, TransmitOptions, ZWaveError, ZWaveErrorCodes, decryptAES128CCM, encodeBitMask, encodeCCList, encryptAES128CCM, getCCName, highResTimestamp, isLongRangeNodeId, isTransmissionError, isZWaveError, parseBitMask, parseCCList, securityClassIsS2, securityClassOrder, validatePayload, } from "@zwave-js/core";
...
import { MGRPExtension, MOSExtension, MPANExtension, SPANExtension, Security2Extension, ValidateS2ExtensionResult, validateS2Extension, } from "../lib/Security2/Extension.js";
...
function securityClassToBitMask(key) {
    return encodeBitMask([key], SecurityClass.S0_Legacy, SecurityClass.S2_Unauthenticated);
}
function bitMaskToSecurityClass(buffer, offset) {
    const keys = parseBitMask(buffer.subarray(offset, offset + 1), SecurityClass.S2_Unauthenticated);
...
function getAuthenticationData(sendingNodeId, destination, homeId, commandLength, unencryptedPayload) {
    const nodeIdSize = isLongRangeNodeId(sendingNodeId) || isLongRangeNodeId(destination)
        ? 2
        : 1;
...
function validateSequenceNumber(securityManager, sourceNodeId, sequenceNumber) {
    validatePayload.withReason(`Duplicate command (sequence number ${sequenceNumber})`)(!securityManager.isDuplicateSinglecast(sourceNodeId, sequenceNumber));
...
function assertSecurityRX(ctx) {
    if (!ctx.ownNodeId) {
        throw new ZWaveError(`Secure commands (S2) can only be decoded when the controller's node id is known!`, ZWaveErrorCodes.Driver_NotReady);
    }
    const ret = getSecurityManager(ctx.ownNodeId, ctx, ctx.sourceNodeId);
    if (!ret) {
        throw new ZWaveError(`Secure commands (S2) can only be decoded when the security manager is set up!`, ZWaveErrorCodes.Driver_NoSecurity);
    }
    return ret;
}
function assertSecurityTX(ctx, destination) {
    if (!ctx.ownNodeId) {
        throw new ZWaveError(`Secure commands (S2) can only be sent when the controller's node id is known!`, ZWaveErrorCodes.Driver_NotReady);
    }
    const ret = getSecurityManager(ctx.ownNodeId, ctx, destination);
    if (!ret) {
        throw new ZWaveError(`Secure commands (S2) can only be sent when the security manager is set up!`, ZWaveErrorCodes.Driver_NoSecurity);
    }
    return ret;
}
async function decryptSinglecast(ctx, securityManager, sendingNodeId, curSequenceNumber, prevSequenceNumber, ciphertext, authData, authTag, spanState, extensions) {
    const decryptWithNonce = async (nonce) => {
        const { keyCCM: key } = securityManager.getKeysForNode(sendingNodeId);
        const iv = nonce;
        const ret = await decryptAES128CCM(ciphertext, key, iv, authData, authTag);
        return {
            key,
            iv,
            ...ret,
    ...
    if (spanState.type === SPANState.SPAN) {
        ...
        const possibleSecurityClasses = isBootstrappingNode
            ...
            : securityClassOrder.filter((s) => ctx.hasSecurityClass(sendingNodeId, s)
                !== false);
        for (const secClass of possibleSecurityClasses) {
            ...
            if (ret.authOK) {
                ...
                if (ctx.hasSecurityClass(sendingNodeId, secClass)
                    === undefined) {
...
function parseExtensions(buffer, wasEncrypted) {
    ...
    parsing: while (true) {
        ...
        const { actual: actualLength, expected: expectedLength } = Security2Extension
...
function getDestinationIDTX() {
    ...
    if (ret == undefined) {
        throw new ZWaveError("Multicast Security S2 encapsulation requires the MGRP extension", ZWaveErrorCodes.Security2CC_MissingExtension);
    }
    return ret;
}
...
let Security2CCAPI = (() => {
    let _classDecorators = [API(CommandClasses["Security 2"])];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = CCAPI;
    var Security2CCAPI = class extends _classSuper {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            Security2CCAPI = _classThis = _classDescriptor.value;
         ...
         */
        async sendNonce() {
            this.assertSupportsCommand(Security2Command, Security2Command.NonceReport);
        ...
        async sendMPAN(groupId, innerMPANState) {
            this.assertSupportsCommand(Security2Command, Security2Command.MessageEncapsulation);
            this.assertPhysicalEndpoint(this.endpoint);
        ...
        async getSupportedCommands(securityClass) {
            this.assertSupportsCommand(Security2Command, Security2Command.CommandsSupportedGet);
            ...
            if (MultiChannelCC.requiresEncapsulation(cc)) {
                const multiChannelCCVersion = this.host.getSupportedCCVersion(CommandClasses["Multi Channel"], this.endpoint.nodeId);
        ...
        async reportSupportedCommands(supportedCCs) {
            this.assertSupportsCommand(Security2Command, Security2Command.CommandsSupportedReport);
        ...
        async requestKeys(params) {
            this.assertSupportsCommand(Security2Command, Security2Command.KEXReport);
        ...
        async sendPublicKey(publicKey, includingNode = true) {
            this.assertSupportsCommand(Security2Command, Security2Command.PublicKeyReport);
        ...
        async requestNetworkKey(securityClass) {
            this.assertSupportsCommand(Security2Command, Security2Command.NetworkKeyGet);
        ...
        async sendNetworkKey(securityClass, networkKey) {
            this.assertSupportsCommand(Security2Command, Security2Command.NetworkKeyReport);
        ...
        async verifyNetworkKey() {
            this.assertSupportsCommand(Security2Command, Security2Command.NetworkKeyVerify);
        ...
        async confirmKeyVerification() {
            this.assertSupportsCommand(Security2Command, Security2Command.TransferEnd);
    ...
    return Security2CCAPI = _classThis;
})();
export { Security2CCAPI };
let Security2CC = (() => {
    let _classDecorators = [commandClass(CommandClasses["Security 2"]), implementedVersion(1)];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = CommandClass;
    var Security2CC = class extends _classSuper {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            Security2CC = _classThis = _classDescriptor.value;
            if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
            __runInitializers(_classThis, _classExtraInitializers);
        }
        async interview(ctx) {
            ...
            const api = CCAPI.create(CommandClasses["Security 2"], ctx, endpoint).withOptions({
            ...
            for (const secClass of possibleSecurityClasses) {
                ...
                if (!securityManager?.hasKeysForSecurityClass(secClass)) {
                    ctx.logNode(node.id, {
                        endpoint: endpoint.index,
                        message: `Cannot query securely supported commands (${getEnumMemberName(SecurityClass, secClass)}) - network key is not configured...`,
                        level: "warn",
                    });
                    continue;
                }
                ctx.logNode(node.id, {
                    endpoint: endpoint.index,
                    message: `Querying securely supported commands (${getEnumMemberName(SecurityClass, secClass)})...`,
                    direction: "outbound",
                });
                ...
                for (let attempts = 1; attempts <= MAX_ATTEMPTS; attempts++) {
                    ...
                    if (supportedCCs == undefined
                        ...
                        if (attempts < MAX_ATTEMPTS) {
                            ...
                            ctx.logNode(node.id, {
                                endpoint: endpoint.index,
                                message: `Querying securely supported commands (${getEnumMemberName(SecurityClass, secClass)}), attempt ${attempts}/${MAX_ATTEMPTS} failed. Retrying in 500ms...`,
                                level: "warn",
                            });
                            await wait(500);
                            continue;
                            ...
                            ctx.logNode(node.id, {
                                endpoint: endpoint.index,
                                message: `Querying securely supported commands (${getEnumMemberName(SecurityClass, secClass)}) failed. Assuming the endpoint supports all its mandatory CCs securely...`,
                                level: "warn",
                            });
                            ...
                            ctx.logNode(node.id, {
                                endpoint: endpoint.index,
                                message: `Querying securely supported commands (${getEnumMemberName(SecurityClass, secClass)}) failed. Let's hope for the best...`,
                                level: "warn",
                            });
                            break;
                        }
                ...
                if (supportedCCs == undefined) {
                    if (endpoint.index === 0
                        ...
                        ctx.logNode(node.id, {
                            message: `The node was NOT granted the security class ${getEnumMemberName(SecurityClass, secClass)}`,
                            direction: "inbound",
                        });
                    }
                    continue;
                ...
                if (endpoint.index === 0 && possibleSecurityClasses.length > 1) {
                    ...
                    ctx.logNode(node.id, {
                        message: `The node was granted the security class ${getEnumMemberName(SecurityClass, secClass)}`,
                        direction: "inbound",
                    });
                }
                if (!hasReceivedSecureCommands && supportedCCs.length > 0) {
                    ...
                    const logLines = [
                        `received secure commands (${getEnumMemberName(SecurityClass, secClass)})`,
                        "supported CCs:",
                    ];
        ...
        static encapsulate(cc, ownNodeId, securityManagers, options) {
            ...
            if (options?.multicastGroupId != undefined) {
                extensions.push(new MGRPExtension({ groupId: options.multicastGroupId }));
            }
...
function getCCResponseForMessageEncapsulation(ctx, sent) {
    if (sent.encapsulated?.expectsCCResponse(ctx)) {
        const ret = [
            Security2CCMessageEncapsulation,
            Security2CCNonceReport,
        ];
        if (sent.encapsulated instanceof Security2CCKEXSet
...
let Security2CCMessageEncapsulation = (() => {
    let _classDecorators = [CCCommand(Security2Command.MessageEncapsulation), expectedCCResponse(getCCResponseForMessageEncapsulation, testCCResponseForMessageEncapsulation)];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = Security2CC;
    var Security2CCMessageEncapsulation = class extends _classSuper {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            Security2CCMessageEncapsulation = _classThis = _classDescriptor.value;
            if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
            __runInitializers(_classThis, _classExtraInitializers);
        }
        constructor(options) {
            ...
            if (!options.encapsulated && !options.extensions?.length) {
                throw new ZWaveError("Security S2 encapsulation requires an encapsulated CC and/or extensions", ZWaveErrorCodes.Argument_Invalid);
            }
        ...
        static async from(raw, ctx) {
            ...
            validatePayload.withReason("No security class granted")(ctx.getHighestSecurityClass(ctx.sourceNodeId) !== SecurityClass.None);
            ...
            const mcctx = (() => {
                ...
                if (ctx.frameType === "multicast" || ctx.frameType === "broadcast") {
                    if (multicastGroupId == undefined) {
                        validatePayload.fail("Multicast frames without MGRP extension");
                    }
                    return {
                        isMulticast: true,
            ...
            const messageLength = 2 /* CommandClass.computeEncapsulationOverhead() */
                + raw.payload.length;
            ...
            if (mcctx.isMulticast) {
                ...
                if (mpanState?.type !== MPANState.MPAN) {
                    ...
                    securityManager.storePeerMPAN(ctx.sourceNodeId, mcctx.groupId, { type: MPANState.OutOfSync });
                    failNoMPAN();
                }
                decrypt = () => decryptMulticast(ctx.sourceNodeId, securityManager, mcctx.groupId, ciphertext, authData, authTag);
            }
            else {
                ...
                decrypt = () => decryptSinglecast(ctx, securityManager, ctx.sourceNodeId, sequenceNumber, prevSequenceNumber, ciphertext, authData, authTag, spanState, extensions);
            }
            let plaintext;
            let authOK = false;
            let key;
            ...
            if (!authOK || !plaintext) {
                if (mcctx.isMulticast) {
                    ...
                    securityManager.storePeerMPAN(ctx.sourceNodeId, mcctx.groupId, { type: MPANState.OutOfSync });
                    validatePayload.fail(ZWaveErrorCodes.Security2CC_CannotDecodeMulticast);
                }
                else {
            ...
            const mpanExtension = extensions.find((e) => e instanceof MPANExtension);
            ...
            if (!mcctx.isMulticast) {
                const mpanExtension = extensions.find((e) => e instanceof MPANExtension);
                if (mpanExtension) {
                    securityManager.storePeerMPAN(ctx.sourceNodeId, mpanExtension.groupId, {
                        type: MPANState.MPAN,
        ...
        async maybeAddSPANExtension(ctx, securityManager) {
            ...
            if (spanState.type === SPANState.None
                ...
                throw new ZWaveError(`Security S2 CC requires the receiver's nonce to be sent!`, ZWaveErrorCodes.Security2CC_NoSPAN);
            }
            else if (spanState.type === SPANState.RemoteEI) {
                ...
                if (this.securityClass == undefined
                    ...
                    if (securityClass == undefined) {
                        throw new ZWaveError("No security class defined for this command!", ZWaveErrorCodes.Security2CC_NoSPAN);
                    }
                    await securityManager.initializeSPAN(receiverNodeId, securityClass, senderEI, receiverEI);
                }
                // Add or update the SPAN extension
                let spanExtension = this.extensions.find((e) => e instanceof SPANExtension);
                if (spanExtension) {
        ...
        async serialize(ctx) {
            ...
            if (this.isSinglecast()) {
                ...
                this.securityClass != undefined
                    ? securityManager.getKeysForSecurityClass(this.securityClass)
            ...
            const { ciphertext: ciphertextPayload, authTag } = await encryptAES128CCM(plaintextPayload, key, iv, authData, SECURITY_S2_AUTH_TAG_LENGTH);
        ...
        toLogEntry(ctx) {
            ...
            if (this.securityClass != undefined) {
                message["security class"] = getEnumMemberName(SecurityClass, this.securityClass);
            }
            return {
    ...
    return Security2CCMessageEncapsulation = _classThis;
})();
export { Security2CCMessageEncapsulation };
let Security2CCNonceReport = (() => {
    let _classDecorators = [CCCommand(Security2Command.NonceReport)];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = Security2CC;
    var Security2CCNonceReport = class extends _classSuper {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            Security2CCNonceReport = _classThis = _classDescriptor.value;
            if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
            __runInitializers(_classThis, _classExtraInitializers);
        }
        constructor(options) {
    ...
    return Security2CCNonceReport = _classThis;
})();
export { Security2CCNonceReport };
let Security2CCNonceGet = (() => {
    let _classDecorators = [CCCommand(Security2Command.NonceGet), expectedCCResponse(Security2CCNonceReport)];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = Security2CC;
    var Security2CCNonceGet = class extends _classSuper {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            Security2CCNonceGet = _classThis = _classDescriptor.value;
            if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
            __runInitializers(_classThis, _classExtraInitializers);
        }
    ...
    return Security2CCNonceGet = _classThis;
})();
export { Security2CCNonceGet };
let Security2CCKEXReport = (() => {
    let _classDecorators = [CCCommand(Security2Command.KEXReport)];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = Security2CC;
    var Security2CCKEXReport = class extends _classSuper {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            Security2CCKEXReport = _classThis = _classDescriptor.value;
            if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
            __runInitializers(_classThis, _classExtraInitializers);
        }
        constructor(options) {
        ...
        static from(raw, ctx) {
            ...
            const supportedKEXSchemes = parseBitMask(raw.payload.subarray(1, 2), 0).filter((s) => s !== 0);
            const supportedECDHProfiles = parseBitMask(raw.payload.subarray(2, 3), ECDHProfiles.Curve25519);
            const requestedKeys = parseBitMask(raw.payload.subarray(3, 4), SecurityClass.S2_Unauthenticated);
            return new this({
                nodeId: ctx.sourceNodeId,
                requestCSA,
                echo,
        ...
        serialize(ctx) {
            this.payload = Bytes.concat([
                ...
                encodeBitMask(this.supportedECDHProfiles, 7, ECDHProfiles.Curve25519),
                encodeBitMask(this.requestedKeys, SecurityClass.S0_Legacy, SecurityClass.S2_Unauthenticated),
            ]);
            return super.serialize(ctx);
        }
        toLogEntry(ctx) {
    ...
    return Security2CCKEXReport = _classThis;
})();
export { Security2CCKEXReport };
let Security2CCKEXGet = (() => {
    let _classDecorators = [CCCommand(Security2Command.KEXGet), expectedCCResponse(Security2CCKEXReport)];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = Security2CC;
    var Security2CCKEXGet = class extends _classSuper {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            Security2CCKEXGet = _classThis = _classDescriptor.value;
            if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
            __runInitializers(_classThis, _classExtraInitializers);
        }
    };
    return Security2CCKEXGet = _classThis;
})();
export { Security2CCKEXGet };
function getExpectedResponseForKEXSet(ctx, sent) {
    if (sent.echo) {
...
let Security2CCKEXSet = (() => {
    let _classDecorators = [CCCommand(Security2Command.KEXSet), expectedCCResponse(getExpectedResponseForKEXSet, testExpectedResponseForKEXSet)];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = Security2CC;
    var Security2CCKEXSet = class extends _classSuper {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            Security2CCKEXSet = _classThis = _classDescriptor.value;
            if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
            __runInitializers(_classThis, _classExtraInitializers);
        }
        constructor(options) {
        ...
        static from(raw, ctx) {
            ...
            const echo = !!(raw.payload[0] & 0b1);
            // The bit mask starts at 0, but bit 0 is not used
            const selectedKEXSchemes = parseBitMask(raw.payload.subarray(1, 2), 0).filter((s) => s !== 0);
            validatePayload(selectedKEXSchemes.length === 1);
            const selectedKEXScheme = selectedKEXSchemes[0];
            const selectedECDHProfiles = parseBitMask(raw.payload.subarray(2, 3), ECDHProfiles.Curve25519);
            validatePayload(selectedECDHProfiles.length === 1);
            const selectedECDHProfile = selectedECDHProfiles[0];
            const grantedKeys = parseBitMask(raw.payload.subarray(3, 4), SecurityClass.S2_Unauthenticated);
            return new this({
                nodeId: ctx.sourceNodeId,
                _reserved,
                permitCSA,
        ...
        serialize(ctx) {
            this.payload = Bytes.concat([
                ...
                encodeBitMask([this.selectedECDHProfile], 7, ECDHProfiles.Curve25519),
                encodeBitMask(this.grantedKeys, SecurityClass.S0_Legacy, SecurityClass.S2_Unauthenticated),
            ]);
            return super.serialize(ctx);
        }
        toLogEntry(ctx) {
            return {
                ...
                message: {
                    echo: this.echo,
                    "selected scheme": getEnumMemberName(KEXSchemes, this.selectedKEXScheme),
                    "selected ECDH profile": getEnumMemberName(ECDHProfiles, this.selectedECDHProfile),
    ...
    return Security2CCKEXSet = _classThis;
})();
export { Security2CCKEXSet };
let Security2CCKEXFail = (() => {
    let _classDecorators = [CCCommand(Security2Command.KEXFail)];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = Security2CC;
    var Security2CCKEXFail = class extends _classSuper {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            Security2CCKEXFail = _classThis = _classDescriptor.value;
            if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
            __runInitializers(_classThis, _classExtraInitializers);
        }
        constructor(options) {
    ...
    return Security2CCKEXFail = _classThis;
})();
export { Security2CCKEXFail };
let Security2CCPublicKeyReport = (() => {
    let _classDecorators = [CCCommand(Security2Command.PublicKeyReport)];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = Security2CC;
    var Security2CCPublicKeyReport = class extends _classSuper {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            Security2CCPublicKeyReport = _classThis = _classDescriptor.value;
            if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
            __runInitializers(_classThis, _classExtraInitializers);
        }
        constructor(options) {
    ...
    return Security2CCPublicKeyReport = _classThis;
})();
export { Security2CCPublicKeyReport };
let Security2CCNetworkKeyReport = (() => {
    let _classDecorators = [CCCommand(Security2Command.NetworkKeyReport)];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = Security2CC;
    var Security2CCNetworkKeyReport = class extends _classSuper {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            Security2CCNetworkKeyReport = _classThis = _classDescriptor.value;
            if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
            __runInitializers(_classThis, _classExtraInitializers);
        }
        constructor(options) {
        ...
        toLogEntry(ctx) {
            return {
                ...super.toLogEntry(ctx),
                message: {
                    "security class": getEnumMemberName(SecurityClass, this.grantedKey),
    ...
    return Security2CCNetworkKeyReport = _classThis;
})();
export { Security2CCNetworkKeyReport };
let Security2CCNetworkKeyGet = (() => {
    let _classDecorators = [CCCommand(Security2Command.NetworkKeyGet)];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = Security2CC;
    var Security2CCNetworkKeyGet = class extends _classSuper {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            Security2CCNetworkKeyGet = _classThis = _classDescriptor.value;
            if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
            __runInitializers(_classThis, _classExtraInitializers);
        }
        constructor(options) {
        ...
        toLogEntry(ctx) {
            return {
                ...super.toLogEntry(ctx),
                message: {
                    "security class": getEnumMemberName(SecurityClass, this.requestedKey),
                },
            };
        }
    };
    return Security2CCNetworkKeyGet = _classThis;
})();
export { Security2CCNetworkKeyGet };
let Security2CCNetworkKeyVerify = (() => {
    let _classDecorators = [CCCommand(Security2Command.NetworkKeyVerify)];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = Security2CC;
    var Security2CCNetworkKeyVerify = class extends _classSuper {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            Security2CCNetworkKeyVerify = _classThis = _classDescriptor.value;
            if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
            __runInitializers(_classThis, _classExtraInitializers);
        }
    };
    return Security2CCNetworkKeyVerify = _classThis;
})();
export { Security2CCNetworkKeyVerify };
let Security2CCTransferEnd = (() => {
    let _classDecorators = [CCCommand(Security2Command.TransferEnd)];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = Security2CC;
    var Security2CCTransferEnd = class extends _classSuper {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            Security2CCTransferEnd = _classThis = _classDescriptor.value;
            if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
            __runInitializers(_classThis, _classExtraInitializers);
        }
        constructor(options) {
    ...
    return Security2CCTransferEnd = _classThis;
})();
export { Security2CCTransferEnd };
let Security2CCCommandsSupportedReport = (() => {
    let _classDecorators = [CCCommand(Security2Command.CommandsSupportedReport)];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = Security2CC;
    var Security2CCCommandsSupportedReport = class extends _classSuper {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            Security2CCCommandsSupportedReport = _classThis = _classDescriptor.value;
            if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
            __runInitializers(_classThis, _classExtraInitializers);
        }
        constructor(options) {
    ...
    return Security2CCCommandsSupportedReport = _classThis;
})();
export { Security2CCCommandsSupportedReport };
let Security2CCCommandsSupportedGet = (() => {
    let _classDecorators = [CCCommand(Security2Command.CommandsSupportedGet), expectedCCResponse(Security2CCCommandsSupportedReport)];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = Security2CC;
    var Security2CCCommandsSupportedGet = class extends _classSuper {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            Security2CCCommandsSupportedGet = _classThis = _classDescriptor.value;
            if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
            __runInitializers(_classThis, _classExtraInitializers);
        }
    };
    return Security2CCCommandsSupportedGet = _classThis;
})();
export { Security2CCCommandsSupportedGet };
