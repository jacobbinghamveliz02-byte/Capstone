...
import * as __validateArgs from "./BarrierOperatorCC._validateArgs.js";
...
import { BarrierOperatorCCValues } from "./_CCValues.generated.js";
export { BarrierOperatorCCValues };
...
function getCCResponseForBarrierOperatorCCSet(ctx, sent) {
    ...
    if (sent.isSinglecast()
        && !sent.isEncapsulatedWith(CommandClasses.Supervision)
        && ctx.getNode(sent.nodeId)?.supportsCC(CommandClasses["Application Status"])) {
        return ApplicationStatusCCRejectedRequest;
    }
}
let BarrierOperatorCCSet = (() => {
    let _classDecorators = [CCCommand(BarrierOperatorCommand.Set), expectedCCResponse(getCCResponseForBarrierOperatorCCSet), useSupervision()];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
