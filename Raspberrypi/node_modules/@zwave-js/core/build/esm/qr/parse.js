...
import { ProvisioningInformationType, QRCodeVersion, minQRCodeLength, onlyDigitsRegex, } from "./definitions.js";
 ...
 */
async function computeChecksum(qr, endOffset = qr.length) {
...
async function parseQRCodeStringInternal(qr, parseSubsets) {
    if (!qr.startsWith("90"))
    ...
    while (offset + 4 <= qr.length) {
        ...
        if (offset + 4 + tlvLength > qr.length)
            break;
 ...
 */
export async function parseQRCodeString(qr) {
