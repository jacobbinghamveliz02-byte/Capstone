...
const { decryptAES128OFB, encryptAES128CBC, encryptAES128ECB, encryptAES128OFB, encryptAES128CCM, decryptAES128CCM, decryptAES256CBC, randomBytes, digest, hmacSHA256, encryptChaCha20Poly1305, decryptChaCha20Poly1305, generateECDHKeyPair, deriveSharedECDHSecret, keyPairFromRawECDHPrivateKey, } = primitives;
export { decryptAES128CCM, decryptAES128OFB, decryptAES256CBC, decryptChaCha20Poly1305, deriveSharedECDHSecret, digest, encryptAES128CBC, encryptAES128CCM, encryptAES128ECB, encryptAES128OFB, encryptChaCha20Poly1305, generateECDHKeyPair, hmacSHA256, keyPairFromRawECDHPrivateKey, randomBytes, };
...
export async function computeMAC(authData, key, iv = new Uint8Array(BLOCK_SIZE).fill(0)) {
...
export async function computeCMAC(message, key) {
    const blockSize = 16;
    ...
    if (!lastBlockIsComplete) {
        lastBlock = zeroPad(Bytes.concat([lastBlock, [0x80]]), blockSize).output;
    }
...
export function computePRK(ecdhSharedSecret, pubKeyA, pubKeyB) {
...
export async function deriveTempKeys(PRK) {
...
export async function deriveNetworkKeys(PNK) {
...
export function computeNoncePRK(senderEI, receiverEI) {
    const message = Bytes.concat([senderEI, receiverEI]);
    return computeCMAC(message, constantNonce);
}
/** Derives the MEI from the nonce PRK */
export async function deriveMEI(noncePRK) {
    const T1 = await computeCMAC(Bytes.concat([
        constantEI,
        [0x00],
        constantEI,
        [0x01],
    ]), noncePRK);
    const T2 = await computeCMAC(Bytes.concat([T1, constantEI, [0x02]]), noncePRK);
