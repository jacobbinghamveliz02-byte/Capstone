...
import { NoiseHandshakeState, encodeNoiseFrame, parseServerHello, } from "../esphome/noise/NoiseProtocol.js";
...
export function createESPHomeFactory(options) {
    return async function () {
        ...
        if (useEncryption) {
            ...
            if (psk.length !== 32) {
                throw new ZWaveError(`Invalid encryption key length: expected 32 bytes, got ${psk.length}`, ZWaveErrorCodes.Driver_InvalidOptions);
            }
        }
        let deviceInfo;
        ...
        async function sendMessage(message) {
            if (useEncryption) {
                if (!sendCipher) {
                    throw new ZWaveError("Cannot send encrypted message: handshake not complete", ZWaveErrorCodes.Driver_NotReady);
                }
                ...
                const encrypted = await sendCipher.encryptWithAd(new Bytes(0), messageData);
        ...
        async function performNoiseHandshake(frameReader) {
            ...
            const readNoiseFrame = (timeoutMs) => readWithTimeout(frameReader, timeoutMs);
            ...
            if (serverHello.protocolVersion !== 0x01) {
                throw new ZWaveError(`Unsupported Noise protocol version: ${serverHello.protocolVersion}`, ZWaveErrorCodes.Driver_InvalidOptions);
            }
            // Send handshake message 1 (prefixed with 0x00 success byte)
            const handshakeState = new NoiseHandshakeState(psk);
            const handshakeMsg1 = await handshakeState.writeMessage(new Bytes(0));
            ...
            if (handshakeMsg2Payload.length === 0) {
                throw new ZWaveError("Noise handshake failed: empty response", ZWaveErrorCodes.Driver_SerialPortClosed);
            }
            ...
            if (statusByte === 0x01) {
                ...
                throw new ZWaveError(`Noise handshake failed: ${errorMessage}`, ZWaveErrorCodes.Driver_SerialPortClosed);
            }
            else if (statusByte !== 0x00) {
                throw new ZWaveError(`Noise handshake failed: unexpected status byte 0x${statusByte.toString(16).padStart(2, "0")}`, ZWaveErrorCodes.Driver_SerialPortClosed);
            }
        ...
        async function performESPHomeHandshake(reader) {
            ...
            async function readExpected(expectedType) {
                while (true) {
                    ...
                    throw new ZWaveError(`Expected ${expectedType.name}, got ${message?.constructor.name}`, ZWaveErrorCodes.Driver_SerialPortClosed);
                }
            }
            // Send HelloRequest
        ...
        async function readWithTimeout(reader, timeoutMs) {
            const timeoutPromise = new Promise((_, reject) => {
                setTimeout(() => reject(new ZWaveError(`Timeout waiting for message`, ZWaveErrorCodes.Driver_SerialPortClosed)), timeoutMs);
            });
            const readPromise = reader.read().then(({ value, done }) => {
                if (done || value === undefined) {
                    throw new ZWaveError("Stream closed unexpectedly", ZWaveErrorCodes.Driver_SerialPortClosed);
                }
                return value;
            });
            ...
            const onConnect = async () => {
                ...
                try {
                    ...
                    if (useEncryption) {
                        ...
                        const decryptTransform = new NoiseDecryptTransform(receiveCipher);
